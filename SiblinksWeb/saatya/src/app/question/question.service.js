brotServices.factory('QuestionsService', ['$http','$log', function($http, $log) {
  ...
  factory.getQuestionByUserId = function(userId, limit, offset,ordertype,oldQid){
	  	var rs;
	    var promise = $http({
	      method: 'GET',
	      url: NEW_SERVICE_URL +'post/getStudentPosted?uid='+userId+'&limit='+limit+'&offset='+offset+'&orderType='+ ordertype+'&oldQid='+oldQid
	    }).success(function(json) {
	      rs = json;
	      return rs;
	    });
  ...
  factory.countQuestions = function(userId, ordertype){
    var rs;
    var promise = $http({
      method: 'GET',
      url: NEW_SERVICE_URL +'post/countQuestions?uid='+userId+'&orderType='+ ordertype
    }).success(function(json) {
      rs = json;
      return rs;
    });
